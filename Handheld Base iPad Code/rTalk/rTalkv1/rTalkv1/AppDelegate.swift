...
func resetAll() {
    ...
    workingData.radioTab = false;
    workingData.filesTab = false;
    workingData.boosterTab = false;
    workingData.GSEtab = false;
    workingData.CatTab = false;
...
var filesVC = FilesView() // used to show-hide radio tab
var boosterVC = BoosterView() // used to show-hide booster tab
var gseVC = GSEView() // used to show-hide booster tab
var catVC = CatConfigView() // used to show-hide booster tab
...
struct padConfigStruct {  // master configuration
    ...
    var PTKenabled: Int = 1
    var PTKrange: Int = 1600
    var PTKalarm: Int = 1200
    var PLDenabled: Int = 1
    var PCHenabled: Int = 0
    var PCHrange: Int = 500
    var CPUtempAlarm: Int = 160
    var SFUopen: Int = 500
    var SFUclose: Int = 1200
    var SOXopen: Int = 500
    var SOXclose: Int = 1200
    var SFLopen: Int = 500
    var SFLclose: Int = 1200
    var SPGopen: Int = 500
    var SPGclose: Int = 1200
    var confirmed: Bool = false
    var refresh: Bool = false
    var staticFire: Bool = true
...
struct padStatusStruct {  // Main variables sent over radio to indicate pad status
    ...
    var SFL: Int = 0
    var SPG: Int = 0
    var processFill60: Int = 0
    var processLaunch: Int = 0
    var processPurge: Int = 0
    var CPUtemp: Int = 0
    var refresh: Bool = false
    var pressureRefresh: Bool = false
    
}

struct GSEworkingStruct {
    var viewHealthRefresh: Bool = false
    var PTKalarm: Int = 0  // 0 = off or reset, 1 = on, 2 = already activated
    var status: String = ""
    var statusTimeout: Date = Date()
    var configRetry: Date = Date()
    var fillTimerStart: Date = Date()
    var purgeTimerStart: Date = Date()
    var purgeTimer: Bool = false
    var fillTimer: Bool = false
    ...
    var iPadBatt: Int = 0
    var radioOwedRight: Bool = false
    var radioLeft: String = ""
    var radioLast: Date = Date()
    var radioTimeout: Bool = false
}

var padStatus = padStatusStruct()
var GSEworking = GSEworkingStruct()
var padConfig = padConfigStruct()
var GSEerrors = GSEerrorsStruct()

let synthesizer = AVSpeechSynthesizer()


// ****************************************************  GSE RADIO PROCESSING *****************************************

func GSEprocessRadio(theData: String) -> Int {
    
    var malformed: Bool = false
    
    //now check for length
    if(theData.count > 180 || theData.count < 10) {
        malformed = true
        workingData.radioRXarray.append("GSE MALFORMED! (size - previous radio sentence")
        let theData:String = "GSE MALFORMED SENTENCE"
        logging(message: theData, filename: workingData.radioLog!)
        workingData.status = "GSE MALFORMED SENTENCE"
        workingData.statusTimeout = (Date() + TimeInterval(configData.statusTime))
        return 0
    }
    ...
    if(malformed == false) {
        ...
        if(!GSEworking.firstContact) {
            GSEworking.firstContact = true
            workingData.status = "GSE RECEIVED FIRST CONTACT"
            workingData.statusTimeout = (Date() + TimeInterval(configData.statusTime))
            print("GSE First Contact");
            workingData.radioSendMessage = "#S,033,!" // this will force a config send
        ...
        if(tempArray[0] == "#FS") { //recieved full status from pad
            ...
            GSEprocessStatus(theArr: tempArray)
            padStatus.refresh = true
            return 1
            } else {
                malformed = true
                workingData.radioRXarray.append("GSE #FS MALFORMED! (size - previous radio sentence")
                let theData:String = "GSE MALFORMED FS SENTENCE"
                logging(message: theData, filename: workingData.radioLog!)
                workingData.status = "GSE MALFORMED FS SENTENCE"
                workingData.statusTimeout = (Date() + TimeInterval(configData.statusTime))
                return 0
            }
        }
        ...
        if(tempArray[0] == "#PRS") { //recieved just pressure status from pad
            ...
            GSEprocessPressure(theArr: tempArray)
            padStatus.pressureRefresh = true
            return 2
        } else {
            malformed = true
            workingData.radioRXarray.append("GSE #PRS MALFORMED! (size - previous radio sentence")
            let theData:String = "GSE MALFORMED PRS SENTENCE"
            logging(message: theData, filename: workingData.radioLog!)
            workingData.status = "GSE MALFORMED PRS SENTENCE"
            workingData.statusTimeout = (Date() + TimeInterval(configData.statusTime))
            return 0
        }
        }
        ...
        if(tempArray[0] == "#CFG") { //recieved full configuration from pad
            ...
            GSEprocessConfig(theArr: tempArray)
            padConfig.refresh = true
            return 2
        } else {
            malformed = true
            workingData.radioRXarray.append("GSE #CFG MALFORMED! (size - previous radio sentence")
            let theData:String = "GSE MALFORMED CONFIG SENTENCE"
            logging(message: theData, filename: workingData.radioLog!)
            workingData.status = "GSE MALFORMED CONFIG SENTENCE"
            workingData.statusTimeout = (Date() + TimeInterval(configData.statusTime))
            return 0
        }
        }
        ...
        if(tempArray[0] == "#HER") { //recieved just pressure status from pad
            if(tempArray.count == 11) { // NEED TO UPDATE == ?
            GSEprocessHealth(theArr: tempArray)
            GSEerrors.refresh = true
            return 2
        } else {
            malformed = true
            workingData.radioRXarray.append("GSE #HER MALFORMED! (size - previous radio sentence")
            let theData:String = "GSE MALFORMED HEALTH SENTENCE"
            logging(message: theData, filename: workingData.radioLog!)
            workingData.status = "GSE MALFORMED HEALTH SENTENCE"
            workingData.statusTimeout = (Date() + TimeInterval(configData.statusTime))
            return 0
        }
        }
        ...
        if(tempArray[0] == "#M") { //got a message
            GSEprocessMessage(theArr: tempArray)
            return 3
        }
        
        if(tempArray[0] == "#DON") { //got a message
            
            workingData.status = "DAQ IS NOW RECORDING"
            workingData.statusTimeout = (Date() + TimeInterval(configData.statusTime))
            if(!GSEworking.DAQrecording) {GSEworking.DAQclock = Date()}
            GSEworking.DAQrecording = true
            padStatus.refresh = true
            return 4
        }
        if(tempArray[0] == "#DOFF") { //got a message
            GSEworking.DAQrecording = false
            workingData.status = "DAQ IS NOW STOPPED"
            workingData.statusTimeout = (Date() + TimeInterval(configData.statusTime))
            padStatus.refresh = true
            return 5
        }
...
func GSEprocessMessage(theArr: [String]) {
    let theNote:String = "GSE: " + theArr[1]
    workingData.status = theNote
    workingData.statusTimeout = (Date() + TimeInterval(configData.statusTime))
    let theData:String = "GSE MESSAGE Received: " + theArr[2]
    logging(message: theData, filename: workingData.flightLog!)
}
...
func GSEprocessStatus(theArr: [String]) {
    
    //speak
    if(intToBool(theVal: theArr[1]) == true && padStatus.rocketConnected == false) { speakNow(theMessage: "Rocket umbilical is now connected") }
    if(intToBool(theVal: theArr[1]) == false && padStatus.rocketConnected == true) { speakNow(theMessage: "Rocket umbilical disconnected") }
    padStatus.rocketConnected = intToBool(theVal: theArr[1])
    
    if(intToBool(theVal: theArr[2]) == true && padStatus.padHot == false) { speakNow(theMessage: "Warning, Pad Controller is now hot") }
    if(intToBool(theVal: theArr[2]) == false && padStatus.padHot == true) { speakNow(theMessage: "Pad Controller is now safe") }
    padStatus.padHot = intToBool(theVal: theArr[2])
    if(intToBool(theVal: theArr[3]) == true && padStatus.armed == false) { speakNow(theMessage: "Pad Controller Armed") }
    if(intToBool(theVal: theArr[3]) == false && padStatus.armed == true) { speakNow(theMessage: "Pad Controller Disarmed") }
    padStatus.armed = intToBool(theVal: theArr[3])
    padStatus.igniterContinuity = intToBool(theVal: theArr[4])
    padStatus.mainVolts = Int(theArr[5]) ?? 0
    padStatus.PTK = Int(theArr[6]) ?? 0 // Tank
    padStatus.PCH = Int(theArr[7]) ?? 0 // Chamber
    padStatus.PLD = Int(theArr[8]) ?? 0 // Load
    padStatus.SFU = Int(theArr[9]) ?? 0 //
    padStatus.SOX = Int(theArr[10]) ?? 0 //
    padStatus.SFL = Int(theArr[11]) ?? 0 //
    padStatus.SPG = Int(theArr[12]) ?? 0 //
    padStatus.processFill60 = Int(theArr[13]) ?? 0 //
    if(padStatus.processFill60 == 2 && !GSEworking.fillTimer) {
        GSEworking.fillTimerStart = Date()
        GSEworking.fillTimer = true
    }
    padStatus.processLaunch = Int(theArr[14]) ?? 0 //
    padStatus.processPurge = Int(theArr[15]) ?? 0 //
    padStatus.CPUtemp = Int(theArr[16]) ?? 0
    
}


func GSEprocessPressure(theArr: [String]) {
    
    padStatus.PTK = Int(theArr[1]) ?? 0 // Ox Tank
    padStatus.PCH = Int(theArr[2]) ?? 0 // Fuel Tank
    padStatus.PLD = Int(theArr[3]) ?? 0  // Pressure ground tank
}


func GSEprocessConfig(theArr: [String]) {
    
    padConfig.PTKenabled = Int(theArr[1]) ?? 0
    padConfig.PTKrange = Int(theArr[2]) ?? 0
    padConfig.PTKalarm = Int(theArr[3]) ?? 0
    padConfig.PLDenabled = Int(theArr[4]) ?? 0
    padConfig.PCHenabled = Int(theArr[5]) ?? 0
    padConfig.PCHrange = Int(theArr[6]) ?? 0
    padConfig.CPUtempAlarm = Int(theArr[7]) ?? 0
    padConfig.SFUopen = Int(theArr[8]) ?? 0
    padConfig.SFUclose = Int(theArr[9]) ?? 0
    padConfig.SOXopen = Int(theArr[10]) ?? 0
    padConfig.SOXclose = Int(theArr[11]) ?? 0
    padConfig.SFLopen = Int(theArr[12]) ?? 0
    padConfig.SFLclose = Int(theArr[13]) ?? 0
    padConfig.SPGopen = Int(theArr[14]) ?? 0
    padConfig.SPGclose = Int(theArr[15]) ?? 0
    padConfig.confirmed = true;
    padConfig.refresh = true;
    
    workingData.status = "GSE Received full configuration"
    workingData.statusTimeout = (Date() + TimeInterval(configData.statusTime))

}

func GSEprocessHealth(theArr: [String]) {
 
    GSEerrors.errorCount = Int(theArr[1]) ?? 0
    GSEerrors.mainBatt = Int(theArr[2]) ?? 0
    GSEerrors.MCUtemp = Int(theArr[3]) ?? 0
    GSEerrors.PT = Int(theArr[4]) ?? 0
    GSEerrors.radio = Int(theArr[5]) ?? 0
    GSEerrors.SD = Int(theArr[6]) ?? 0
    GSEerrors.ADC = Int(theArr[7]) ?? 0
    GSEerrors.MCUcrash = Int(theArr[8]) ?? 0
    GSEerrors.Fuse = Int(theArr[9]) ?? 0
