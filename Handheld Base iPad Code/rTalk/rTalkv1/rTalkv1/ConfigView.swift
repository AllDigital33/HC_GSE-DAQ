...
class ConfigView: UIViewController {
    ...
    @IBOutlet weak var switchFilesTab: UISwitch!
    @IBOutlet weak var switchBoosterTab: UISwitch!
    @IBOutlet weak var switchGSETab: UISwitch!
    @IBOutlet weak var switchCatTab: UISwitch!
    ...
    func refreshConfig() {
        ...
        if(workingData.radioTab ?? true) {
            switchRadioTab.isOn = true
        } else {
            switchRadioTab.isOn = false
        }
        if(workingData.filesTab ?? true) {
            switchFilesTab.isOn = true
        } else {
            switchFilesTab.isOn = false
        }
        if(workingData.boosterTab ?? true) {
            switchBoosterTab.isOn = true
        } else {
            switchBoosterTab.isOn = false
        }
        if(workingData.GSEtab ?? true) {
            switchGSETab.isOn = true
        } else {
            switchGSETab.isOn = false
        }
        if(workingData.CatTab ?? true) {
            switchCatTab.isOn = true
        } else {
            switchCatTab.isOn = false
        }
    ...
    @IBAction func switchRadioTabChange(_ sender: Any) {
        if(switchRadioTab.isOn == false) { //hide the tab
            var theIndex: Int = 0
            ...
            if(!test) {
              tabBarController?.viewControllers?.insert(radioVC,at: theIndex)
                workingData.radioTab = true
            }
        }
    }
    
    @IBAction func switchFilesTabChange(_ sender: Any) {
        if(switchFilesTab.isOn == false) { //hide the tab
            var theIndex: Int = 0
            if let viewControllers = tabBarController?.viewControllers {
                for viewController in viewControllers {
                    let className = String(describing: Mirror(reflecting: viewController).subjectType)
                    if className.contains("Files") {
                        print("Found a 'DAQViewController' instance.")
                        filesVC = tabBarController?.viewControllers?.remove(at: theIndex) as! FilesView
            ...
            if(!test) {
              tabBarController?.viewControllers?.insert(filesVC,at: theIndex)
                workingData.filesTab = true
            }
        }
    }
    
    
    @IBAction func switchBoosterTabChange(_ sender: Any) {
        if(switchBoosterTab.isOn == false) { //hide the tab
            var theIndex: Int = 0
            if let viewControllers = tabBarController?.viewControllers {
                for viewController in viewControllers {
                    let className = String(describing: Mirror(reflecting: viewController).subjectType)
                    if className.contains("Booster") {
                        print("Found a 'DAQViewController' instance.")
                        boosterVC = tabBarController?.viewControllers?.remove(at: theIndex) as! BoosterView
            ...
            if(!test) {
              tabBarController?.viewControllers?.insert(boosterVC,at: theIndex)
                workingData.boosterTab = true
            }
        }
    }
    
    @IBAction func switchGSEtabChange(_ sender: Any) {
        if(switchGSETab.isOn == false) { //hide the tab
            var theIndex: Int = 0
            if let viewControllers = tabBarController?.viewControllers {
                for viewController in viewControllers {
                    let className = String(describing: Mirror(reflecting: viewController).subjectType)
                    if className.contains("GSE") {
                        print("Found a 'GSE controller' instance.")
                        gseVC = tabBarController?.viewControllers?.remove(at: theIndex) as! GSEView
            ...
            if(!test) {
              tabBarController?.viewControllers?.insert(gseVC,at: theIndex)
                workingData.GSEtab = true
            }
        }
    }
    
    @IBAction func switchCatConfigTabChange(_ sender: Any) {
        if(switchCatTab.isOn == false) { //hide the tab
            var theIndex: Int = 0
            if let viewControllers = tabBarController?.viewControllers {
                for viewController in viewControllers {
                    let className = String(describing: Mirror(reflecting: viewController).subjectType)
                    if className.contains("Cat") {
                        print("Found a 'Cat Config' instance.")
                        catVC = tabBarController?.viewControllers?.remove(at: theIndex) as! CatConfigView
            ...
            print("showing the Cat Config view")
            //tabBarController?.viewControllers?.insert(radioVC,at: 6)
            var theIndex: Int = 0
            var test: Bool = false
            if let viewControllers = tabBarController?.viewControllers {
            ...
            if(!test) {
              tabBarController?.viewControllers?.insert(catVC,at: theIndex)
                workingData.CatTab = true
            }
        }
    }
