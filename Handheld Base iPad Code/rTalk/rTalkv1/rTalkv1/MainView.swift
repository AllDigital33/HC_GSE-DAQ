...
class MainView: UIViewController, CLLocationManagerDelegate, RedSerialPortDelegate, RedSerialDeviceManagerDelegate {
        ...
        if let viewControllers = tabBarController?.viewControllers {
            for viewController in viewControllers {
                ...
                if className.contains("Files") {
                    ...
                    filesVC = tabBarController?.viewControllers?.remove(at: theIndex2) as! FilesView
                }
             theIndex2 += 1
            }
        }
        ...
        if let viewControllers = tabBarController?.viewControllers {
            for viewController in viewControllers {
                ...
                if className.contains("Booster") {
                    ...
                    boosterVC = tabBarController?.viewControllers?.remove(at: theIndex3) as! BoosterView
                }
             theIndex3 += 1
            }
        }
        ...
        if let viewControllers = tabBarController?.viewControllers {
            for viewController in viewControllers {
                ...
                if className.contains("GSE") {
                    ...
                    gseVC = tabBarController?.viewControllers?.remove(at: theIndex4) as! GSEView
                }
             theIndex4 += 1
            }
        }
        ...
        if let viewControllers = tabBarController?.viewControllers {
            for viewController in viewControllers {
                ...
                if className.contains("Cat") {
                    ...
                    catVC = tabBarController?.viewControllers?.remove(at: theIndex5) as! CatConfigView
                }
             theIndex5 += 1
            }
        }
    ...
    func GSEradio(theData: String) {
        
        //Now Log the GSE radio line
        GSEworking.radioOwedRight = false //reset this
        let now = Date()
        ...
        workingData.radioRXarray.append(String(datetime + "  " + theData))
        logging(message: theData, filename: workingData.radioLog!)
        
        let radioProcess: Int = GSEprocessRadio(theData: theData)
        if (radioProcess != 0) {
            GSEworking.radioLast = Date()
            GSEworking.radioTimeout = false
        }
        
        if(configData.alertRadioRX) {self.playSound(theFile: "cowbell")}
        self.doReceive()
    }
    
    func GSEradioLeft(theData: String) {
        GSEworking.radioOwedRight = true
        GSEworking.radioLeft = theData
        self.doReceive()
    }
    func GSEradioRight(theData: String) {
        var newString: String = ""
        newString = GSEworking.radioLeft + theData
        GSEradio(theData: newString)
        self.doReceive()
    }
    ...
    func doReceive()  // Radio Handler
    {
        ...
        myPort.recvData { [self] (data: Data) in
            ...
            // ------- GSE RADIO INTERCEPT --------
            if (tempLeft == "#" && tempRight=="!") { // This is a GSE packet and not a RT packet
                GSEradio(theData: dataString)
                return
            }
            if (tempLeft == "#" && tempRight != "!") { // This is a left GSE
                GSEradioLeft(theData: dataString)
                return
            }
            if (tempLeft != "#" && tempLeft != "@" && GSEworking.radioOwedRight && tempRight == "!") { // This is a left GSE
                GSEradioRight(theData: dataString)
                return
            }
